...
*)
let rec worklist_first : production -> first_set -> first_set
=fun w a ->
if w = [] then a 
else let (s,l) = List.hd w in
     let tl_of_w = List.tl w in
(
if l=[] then (worklist_first (tl_of_w) (update_set a s Epsilon))
else let hd = List.hd l in
     let tl_of_l = List.tl l in
(match hd with
|(T x) -> (worklist_first (tl_of_w) (update_set a s (T x)))
|(N x) -> if (find_nonterm (N x) tl_of_w ) then (worklist_first (tl_of_w@[(s,l)]) a)
          else let first_x = BatMap.find (N x) a in
            (if (BatSet.mem Epsilon first_x)then (worklist_first (tl_of_w@[(s,tl_of_l)]) (union_set a s (BatSet.remove Epsilon first_x)) )
             else worklist_first (tl_of_w) (union_set a s (first_x)) 
            ) 
...
*)
let rec check_one_pr : first_set -> symbol -> (symbol * symbol list) -> symbol list -> follow_set -> (symbol list * follow_set)
=fun fs x (s,l) w a->
...
(if (after_x_string = []) then (if s = x then 
    ...
    (if (List.mem s w) then let new_w = (if(List.hd w = x) then (List.tl w @[x]) else w@[x]) in
                            (new_w,a)
     else let new_a = (union_set a x (BatMap.find s a)) in 
     ...
     let first_ns = (BatMap.find next_symbol fs) in
     if(BatSet.mem Epsilon first_ns) then 
          let new_a = (union_set a x (BatSet.remove Epsilon first_ns)) 
          in check_one_pr fs x (s,x::(List.tl after_x_string)) w (new_a) 
...
let rec check_pr : first_set -> symbol -> production -> symbol list -> follow_set -> (symbol list * follow_set)
=fun fs x pr w a->
match pr with
|[] -> (w,a)
|hd::tl ->( let (new_w, new_a) = (check_one_pr fs x hd w a) in
            let (s,l) = hd in
            let after_x_string = (split_list x l) in
            (if (List.mem x after_x_string) then 
            let x_again = x::(split_list x after_x_string) in
            (check_pr fs x (tl@[(s,x_again)]) new_w new_a) else
            (check_pr fs x tl new_w new_a)) )
...
|hd::tl -> (let pr = (List.fold_left (fun x (s,l) -> if (List.mem hd l) then x@[(s,l)] else x) [] p) in
           ...
           else let (new_w, new_a) = (check_pr fs hd pr w a) in 
